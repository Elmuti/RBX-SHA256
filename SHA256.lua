...
elseif bit and bit.band then
	...
	local bit_lua = require(workspace.bit_lua)
	...
	if not bit_lua.version or bit_lua.version[1] ~= bit_lua_target[1] or bit_lua.version[2] ~= bit_lua_target[2] then
		...
		local expected = table.concat(bit_lua_target, ".")
		error(string.format("Expected version %s of the pure Lua bitwise functions, got %s.", expected, required))
	end
